...
# --- Configuration ---
INITIAL_MONEY = 10000
TARGET_NET_WORTH = 25000 # Increased goal for more challenge
...
class Business:
    ...
    # <<< CHANGE: New method for salaries >>>
    def pay_salaries(self):
        """Deducts salaries based on current workers and stores the cost."""
        salary_cost = self.workers * WORKER_SALARY
        self.money -= salary_cost
        self.last_turn_salaries = salary_cost # Store salary cost
        # Return value is optional now, could just be side effect
        return salary_cost

    # <<< CHANGE: New method for interest >>>
    def apply_interest(self, loan_balance_at_start_of_period):
        """Applies interest based on the loan balance at the start of the period."""
        # Reset interest tracker for the turn
        self.last_turn_interest_paid = 0
        interest_earned = 0

        # Interest earned on cash (if positive balance) - This uses current money
        if self.money > 0:
            interest_earned = self.money * (INTEREST_RATE / 12)
            self.money += interest_earned

        # Interest paid on loan - Uses the balance passed in
        if loan_balance_at_start_of_period > 0:
            ...
            self.last_turn_interest_paid = interest_paid # Store interest paid

        # Return values optional, could just be side effect
...
class Game:
    ...
    def get_player_actions(self):
        ...
        # <<< CHANGE: Corrected max_affordable_hires calculation >>>
        calculated_hires = 0 # Default to 0
        if HIRING_COST_PER_WORKER > 0:
            cash_available_for_hiring = player.money - 500 # Leave a small buffer
            # Only calculate if there's actually cash available above the buffer
            if cash_available_for_hiring > 0:
                calculated_hires = int(cash_available_for_hiring // HIRING_COST_PER_WORKER)
            # Otherwise, calculated_hires remains 0
        else: # Handle free hiring case (unlikely but good practice)
             calculated_hires = 1000 # Set an arbitrary high limit if hiring is free

        # Ensure the max value passed to input is never negative
        max_affordable_hires = max(0, calculated_hires)
        # <<< END CHANGE >>>

        # Now max_affordable_hires is guaranteed to be 0 or positive
        ...
        # --- Firing Logic (Unaffected, but ensure affordability checks are robust too) ---
        fire_workers = 0
        if player.workers > 0 :
             max_fires = player.workers
             # Check affordability for firing (severance cost)
             affordable_severance = player.money - 500 # Keep buffer when considering firing too
             max_affordable_fires = max_fires # Assume can fire all initially
             if FIRING_COST_PER_WORKER > 0 and affordable_severance < (max_fires * FIRING_COST_PER_WORKER):
                 if affordable_severance > 0:
                     max_affordable_fires = int(affordable_severance // FIRING_COST_PER_WORKER)
                 else:
                      max_affordable_fires = 0 # Cannot afford to fire anyone if buffer cash is negative

             can_fire = min(max_fires, max_affordable_fires) # Max is limited by affordability

             fire_workers_input = get_int_input(f"How many workers to FIRE? (Max: {player.workers}, Affordable Severance for: {can_fire}) ", 0, player.workers) # Ask up to total workers

             if fire_workers_input > can_fire:
                 print(f"WARN: Cannot afford severance for {fire_workers_input} workers (Affordable: {can_fire}).")
                 fire_workers = 0 # Don't fire if they try to fire more than affordable
                 # Alternative: Fire only the affordable number? Let's prevent it for now.
                 # fire_workers = can_fire # Fire the affordable amount instead
             elif fire_workers_input > 0:
                  fire_workers = fire_workers_input # Proceed with the valid input
    ...
    def process_turn(self):
        ...
        # <<< CHANGE: Capture start-of-turn loan balances >>>
        start_of_turn_loan_balances = {}
        for biz in self.businesses:
            if not biz.bankrupt: # Only need balance for active businesses
                start_of_turn_loan_balances[biz.name] = biz.loan_amount
        ...
        # 5. <<< CHANGE: Apply Salaries and Interest Separately >>>
        print("\nProcessing salaries and interest...")
        # First, pay salaries based on current workers
        for biz in self.businesses[:]:
             if biz.bankrupt: continue
             biz.pay_salaries() # Uses current worker count

        # Then, apply interest based on the captured start-of-turn balance
        for biz in self.businesses[:]:
            if biz.bankrupt: continue
            # Use the stored balance for the interest calculation
    ...
    def run(self):
        """Main game loop."""
        print("\nWelcome to Business Tycoon Simulator v0.8!")
